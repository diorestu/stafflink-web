...
@section('content')
    <div class="max-w-5xl">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            ...
            <form action="{{ route('admin.jobs.update', $job) }}" method="POST" class="p-8 space-y-8">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                        <div>
                            ...
                            <select name="country" id="country" required
                                data-selected="{{ old('country', $job->country) }}"
                        ...
                        <div>
                            ...
                            <select name="state" id="state" required data-selected="{{ old('state', $job->state) }}"
                                class="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#287854] focus:border-transparent bg-white">
                ...
                <section class="space-y-4">
                    <div>
                        <h4 class="text-sm font-semibold tracking-wide text-gray-500 uppercase">Compensation and Publish</h4>
                    </div>
                        ...
                        <div class="space-y-3">
                            <div class="salary-dual-slider py-1">
                                <div class="salary-slider-wrap">
                                    ...
                                    <input type="range" id="minimum_salary_slider" min="0" max="50000000" step="500000"
                                        value="{{ old('minimum_salary', $job->minimum_salary ?? 5000000) }}" class="salary-range-input salary-range-min">
                                    <input type="range" id="maximum_salary_slider" min="0" max="50000000" step="500000"
                                        value="{{ old('maximum_salary', $job->maximum_salary ?? 10000000) }}" class="salary-range-input salary-range-max">
                                </div>
                                <div class="mt-3 flex justify-between text-[34px] leading-none font-semibold text-gray-600">
                                    <span>Min</span>
                                    <span>Max</span>
                                </div>
                                <input type="hidden" name="minimum_salary" id="minimum_salary_input" value="{{ old('minimum_salary', $job->minimum_salary ?? 5000000) }}">
                                <input type="hidden" name="maximum_salary" id="maximum_salary_input" value="{{ old('maximum_salary', $job->maximum_salary ?? 10000000) }}">
                            </div>
                            ...
                            <p class="text-sm text-gray-600">
                                Min: IDR <span id="minimum_salary_value"></span> | Max: IDR <span id="maximum_salary_value"></span>
                            </p>
    ...
    <script>
        (() => {
            ...
            if (!countrySelect || !stateSelect || !titleInput || !descriptionInput || !aiButton || !aiMessage || !aiButtonLabel || !aiSpinner || !minimumSalarySlider || !maximumSalarySlider || !minimumSalaryInput || !maximumSalaryInput || !minimumSalaryValue || !maximumSalaryValue || !salaryRangePreview || !salaryTrackActive) return;
